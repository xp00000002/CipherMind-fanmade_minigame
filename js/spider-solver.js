...
(function (global) {
  ...
  function canMove(p, orig, dest, num) {
    ...
    var srcCard = p.desk[orig][p.desk[orig].length - num];
    var destCard = destCards[destCards.length - 1];
    return srcCard.suit === destCard.suit && srcCard.point + 1 === destCard.point;
  }
  function restore(p) {
    for (var i = 0; i < 10; i++) {
      var col = p.desk[i];
